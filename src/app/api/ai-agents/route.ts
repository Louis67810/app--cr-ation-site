...
export async function POST(request: Request) {
  ...
  if (!isPipelinePhase(payload.phase) || !isEditorialMode(payload.mode) || typeof payload.topic !== "string" || !payload.topic.trim()) {
    return NextResponse.json({ error: "Phase, mode ou sujet invalide." }, { status: 400 });
  }
  ...
  try {
    ...
    if (!research?.summary || !Array.isArray(research.facts)) {
      return NextResponse.json({ error: "Le dossier de recherche est manquant." }, { status: 400 });
    }
    ...
    if (!outline?.title || !Array.isArray(outline.sections)) {
      return NextResponse.json({ error: "Le plan éditorial est manquant." }, { status: 400 });
    }
    ...
    if (payload.mode === "youtube") {
      const visual = await generateArticleVisual(article, source?.slice(0, 180) ?? topic);
      if (visual) {
