...
export async function GET(request: Request) {
  ...
  for (const setting of settings ?? []) {
    ...
    try {
      const [{ data: project }, { data: events }, { data: traffic }, { data: monthlyVisitors }] = await Promise.all([
        ...
        supabase.from("project_site_tracking_visitors").select("visitor_id").eq("owner_id", setting.owner_id).eq("project_key", setting.project_key).gte("day", periodStart).lt("day", currentStartIso.slice(0, 10)),
      ]);
      const activity = (events ?? []) as Array<{ event_type: string; entity_title: string }>;
      const totals = { pages: activity.filter((event) => event.event_type === "page_created" || event.event_type === "article_created" || event.event_type === "realisation_created").length, articles: activity.filter((event) => event.event_type === "article_created").length, realisations: activity.filter((event) => event.event_type === "realisation_created").length, publications: activity.filter((event) => event.event_type === "project_published").length, visitors: new Set((monthlyVisitors ?? []).map((row) => row.visitor_id)).size, pageViews: (traffic ?? []).reduce((sum, row) => sum + Number(row.page_views ?? 0), 0) };
