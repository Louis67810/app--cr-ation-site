...
function LineChart({ series }: { series: Array<{ day: string; pageViews: number; uniqueVisitors: number }> }) {
  ...
  return <div className="mt-7 border-t border-black/[0.08] pt-3">
    <div className="relative h-[270px] w-full overflow-hidden">
      ...
      <div className="absolute bottom-0 left-0 right-0 flex justify-between text-[11px] font-medium text-black/40"><span>{series[0] ? new Intl.DateTimeFormat("fr-FR", { day: "numeric", month: "short" }).format(new Date(`${series[0].day}T12:00:00`)) : "—"}</span><span>{series.at(-1) ? new Intl.DateTimeFormat("fr-FR", { day: "numeric", month: "short" }).format(new Date(`${series.at(-1)!.day}T12:00:00`)) : "—"}</span></div>
    </div>
  </div>;
}
...
export function AnalyticsDashboard({ projectKey, projectOwnerId, initialData }: { projectKey: string; projectOwnerId: string; initialData: EditorialPerformanceSnapshot; gaPropertyId?: string }) {
  ...
  const series = useMemo(() => {
    const days = analytics?.series.map((item) => item.day) ?? [];
    return days.map((day) => {
      const events = filteredEvents.filter((event) => event.occurred_at.slice(0, 10) === day);
      return { day, pageViews: events.length, uniqueVisitors: new Set(events.map((event) => event.visitor_id)).size };
    });
