...
export function MonthlyRecap({ project, data }: { project: DashboardProject; data: MonthlyRecapData }) {
  ...
  const counts = useMemo(() => ({
    ...
    total: initialEvents.filter((event) => event.event_type === "page_created" || event.event_type === "article_created" || event.event_type === "realisation_created").length,
  }), [initialEvents]);
  ...
  return <div className="pb-8 font-[var(--font-inter)] sm:pb-16">
    ...
    <p className="mt-2 text-[10px] text-black/30">{pageViews} vue(s) de pages enregistrée(s). Les visiteurs sont dédupliqués sur tout le mois, même s’ils consultent plusieurs pages.</p>
