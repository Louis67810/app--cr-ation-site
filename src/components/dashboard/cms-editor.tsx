...
const collections = [
  ...
  { id: "pages", label: "Pages", match: (page: SitePage) => !["realisation", "blog", "prestation", "secteur"].some((part) => page.slug.includes(part)) },
] as const;
...
function collectColumns(value: JsonValue, path: Path = [], labels: string[] = []): ContentColumn[] {
  if (Array.isArray(value)) {
    return value.flatMap((child, index) => collectColumns(child, [...path, index], [...labels, `${index + 1}`]));
  }
  if (value && typeof value === "object") {
    return Object.entries(value).flatMap(([key, child]) => hiddenKeys.has(key) ? [] : collectColumns(child, [...path, key], [...labels, humanize(key)]));
  }
  const lastKey = String(path.at(-1) ?? "champ");
  return [{ key: path.join("."), path, label: labels.slice(-3).join(" · "), image: isImage(lastKey) }];
}
...
export function CmsEditor({ project, canOpenBuilder }: { project: CmsProject; canOpenBuilder: boolean }) {
  ...
  async function play() {
    if (await save()) window.open(`/builder?project=${encodeURIComponent(project.key)}`, "_blank", "noopener,noreferrer");
  }
  ...
  return (
    <section className="flex h-full min-h-0 min-w-0 flex-col overflow-hidden bg-white font-[var(--font-inter)] text-[#1c1c1c]">
        <header className="grid min-h-[92px] shrink-0 grid-cols-[1fr_auto] grid-rows-2 items-center gap-x-2 border-b border-[#e8e8e8] px-2 py-1.5 sm:h-12 sm:min-h-0 sm:grid-cols-[1fr_auto_1fr] sm:grid-rows-1 sm:px-3 sm:py-0">
          <Link href={`/dashboard?project=${encodeURIComponent(project.key)}`} className="col-start-1 row-start-1 flex h-8 w-fit items-center gap-2 rounded-[8px] bg-white px-2 text-[11px] font-semibold text-[#222] shadow-[0_1px_2px_rgba(0,0,0,0.08),0_0_0_1px_rgba(0,0,0,0.08)] sm:text-[12px]"><span className="flex size-5 items-center justify-center rounded-[5px] bg-[#f6f6f6] shadow-[inset_0_0_0_1px_rgba(0,0,0,0.08)]"><ArrowLeft size={13} /></span>Retour</Link>
          <div className="relative col-span-2 col-start-1 row-start-2 justify-self-center sm:col-span-1 sm:col-start-2 sm:row-start-1">
            <button type="button" onClick={() => setCollectionOpen((current) => !current)} className="flex items-center gap-2 px-3 py-2 font-serif text-[12px]">
              {collection.label}<ChevronDown size={13} className="opacity-70" />
            </button>
          ...
          <div className="col-start-2 row-start-1 flex items-center justify-end gap-2 sm:col-start-3"><span className="mr-2 hidden text-[10px] text-black/40 xl:block">{message}</span>{canOpenBuilder ? <button type="button" onClick={play} disabled={status !== "idle"} aria-label="Prévisualiser le CMS" className="flex size-8 items-center justify-center rounded-[9px] bg-[#f3f3f3] text-[#222] hover:bg-[#eee] disabled:opacity-50"><Play size={15} fill="currentColor" /></button> : null}<button type="button" onClick={publish} disabled={status !== "idle"} className="flex h-9 w-[98px] items-center justify-center gap-1.5 rounded-[10px] bg-[linear-gradient(180deg,#323232_0%,#222222_100%)] px-3 py-2 text-[12px] font-semibold leading-5 tracking-[-0.02em] text-[#fcfcfc] shadow-[0_2px_4px_-1px_rgba(13,13,13,0.5),0_0_0_1px_#333333,inset_0_0.5px_1px_rgba(255,255,255,0.15),inset_0_-1px_1.2px_0.35px_#121212] hover:brightness-110 disabled:opacity-50 sm:w-[141px] sm:px-5 sm:text-[14px]">{status === "publishing" ? <LoaderCircle size={14} className="animate-spin" /> : null}Publier</button></div>
        </header>
