    ...
    <main className="grid min-h-screen grid-cols-[124px_minmax(0,1fr)] bg-[#303030] p-[14px_22px_20px_29px] font-[var(--font-inter)] text-[#1c1c1c]">
      ...
      <section className="min-w-0 bg-white">
        <header className="grid h-11 grid-cols-[1fr_auto_1fr] items-center border-b border-black/[0.07] px-4">
          ...
          <div className="flex items-center justify-end gap-2"><span className="mr-2 hidden text-[9px] text-black/40 xl:block">{message}</span><button type="button" onClick={play} disabled={status !== "idle"} aria-label="Prévisualiser le CMS" className="grid size-7 place-items-center rounded-[7px] bg-[#f7f7f6] disabled:opacity-50"><Play size={11} fill="currentColor" /></button><button type="button" onClick={publish} disabled={status !== "idle"} className="flex h-7 min-w-[76px] items-center justify-center gap-1.5 rounded-[7px] bg-gradient-to-b from-[#323232] to-[#222] px-4 text-[9px] font-semibold text-white shadow-md disabled:opacity-50">{status === "publishing" ? <LoaderCircle size={11} className="animate-spin" /> : null}Publier</button></div>
        </header>
        ...
        <div className="max-h-[calc(100vh-128px)] overflow-auto">
