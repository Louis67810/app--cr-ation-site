...
export function DashboardShell({ projects, selectedKey }: { projects: DashboardProject[]; selectedKey: string }) {
    ...
    <main className="min-h-screen bg-white text-[#1c1c1c] lg:grid lg:grid-cols-[212px_1fr]">
      <aside className="border-b border-black/10 bg-[#fcf9f4] px-4 py-4 lg:sticky lg:top-0 lg:h-screen lg:border-b-0 lg:border-r">
        ...
        <nav className="mt-10">
          ...
          <div className="mt-2 grid gap-1 text-[14px]">
            <a href="#vue-ensemble" className="relative flex h-8 items-center gap-3 rounded-lg bg-black/5 px-3 pl-9 before:absolute before:left-0 before:h-4 before:w-1 before:rounded-full before:bg-black"><Home size={18} />Vue d’ensemble</a>
            <a href="#activite" className="flex h-8 items-center gap-3 rounded-lg px-3 pl-9 hover:bg-black/5"><BarChart3 size={18} />Activité</a>
            <a href="#pages" className="flex h-8 items-center gap-3 rounded-lg px-3 pl-9 hover:bg-black/5"><FolderKanban size={18} />Pages du site</a>
          </div>
        </nav>
      </aside>
      ...
      <section className="min-w-0 px-5 py-8 sm:px-8 lg:px-10 lg:py-11 xl:px-12">
        <header id="vue-ensemble" className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <h1 className="font-serif text-[30px] tracking-[-0.05em]">Bonjour, voici votre site</h1>
            <p className="mt-2 text-[14px] font-medium text-black/60">Suivez le contenu, les contrôles et la publication de {project.name}.</p>
          </div>
        ...
        <div className="mt-8 flex max-w-[633px] items-center gap-2 rounded-[10px] border border-[#d7dce4] px-3 py-2.5 focus-within:border-black/40">
          <Search size={16} />
        ...
        <div className="mt-10 grid gap-6 xl:grid-cols-[minmax(0,1.05fr)_minmax(360px,.95fr)]">
          <section id="activite">
        ...
        <section id="pages" className="mt-10 pb-12">
