...
type OverlayView = "ideas" | "idea-form" | "idea-detail" | "article" | "images" | "statistics" | "research" | "outline" | "writing" | "quiz" | null;
...
export function AiAgents({ project, initialAnalytics }: { project: DashboardProject; initialAnalytics: EditorialPerformanceSnapshot }) {
  ...
  return <div className="-mx-3 -mt-3 min-h-[1004px] bg-white font-[var(--font-inter)] text-[#1c1c1c] sm:-mx-6 sm:-mt-6">
    ...
    {view ? <Overlay onClose={closeOverlay} size={view === "images" ? "tall" : view === "ideas" || view === "idea-form" || view === "idea-detail" ? "large" : "medium"}>
      ...
      {view === "statistics" && activeArticle ? <StatisticsOverlay page={activeArticle} performance={performanceByPath.get(activeArticle.slug)} analytics={analytics} onBack={() => setView("article")} onClose={closeOverlay} /> : null}
...
function ModePill({ mode }: { mode: IdeaMode }) {
  const styles = mode === "youtube" ? "bg-[#fff0ed] text-[#a84732]" : mode === "trends" ? "bg-[#eef2ff] text-[#4f5e9b]" : "bg-[#ededed] text-black/50";
  return <span className={`rounded-full px-2 py-[5px] text-[11px] leading-4 ${styles}`}>{modeLabel(mode)}</span>;
}
...
function OverlayTop({ title, children, onClose }: { title: string; children?: ReactNode; onClose: () => void }) {
  return <header className="flex min-h-[104px] shrink-0 items-center gap-4 border-b border-black/[0.08] px-5 sm:px-9"><h2 className="min-w-0 flex-1 truncate font-serif text-[24px] leading-none">{title}</h2>{children}<button type="button" onClick={onClose} aria-label="Fermer" className="grid size-10 shrink-0 place-items-center rounded-full bg-[#f3f3f3] text-black/55"><X size={18} /></button></header>;
}
...
function ArticleFolder({ icon, title, detail, onClick, preview }: { icon: ReactNode; title: string; detail?: string; onClick: () => void; preview?: string }) {
  return <button type="button" onClick={onClick} className="flex min-h-20 w-full items-center gap-5 rounded-[20px] border border-black/[0.04] bg-[#fafafa]/30 px-7 text-left transition hover:border-black/10 hover:bg-[#fafafa]"><span className="relative grid size-10 shrink-0 place-items-center text-[#aeaeae]">{preview ? <><span className="absolute left-0 h-9 w-8 -rotate-6 overflow-hidden rounded-[4px] border-2 border-white bg-[#ddd] shadow"><Image src={preview} alt="" fill unoptimized className="object-cover" /></span><span className="absolute right-0 h-9 w-8 rotate-6 overflow-hidden rounded-[4px] border-2 border-white bg-[#ddd] shadow"><Image src={preview} alt="" fill unoptimized className="object-cover" /></span></> : icon}</span><span className="min-w-0"><span className="block font-serif text-[18px]">{title}</span>{detail ? <span className="mt-1 block truncate text-[10px] text-black/35">{detail}</span> : null}</span><ExternalLink size={17} className="ml-auto shrink-0 text-black/25" /></button>;
}

function ImagesOverlay({ page, onBack, onClose }: { page: SitePage; onBack: () => void; onClose: () => void }) {
  const fields = getArticleFields(page);
  const images = [
    ...(fields?.heroImageUrl ? [{ url: fields.heroImageUrl, title: "Image principale", alt: fields.heroImageAlt || page.editorial?.article?.heroImageAlt || getTitle(page) }] : []),
    ...(fields?.blocks.flatMap((block, index) => block.kind === "image" ? [{ url: block.imageUrl, title: `Image ${index + 1}`, alt: block.alt }] : []) ?? []),
  ].filter((image, index, all) => all.findIndex((candidate) => candidate.url === image.url) === index);
  return <><OverlayTop title={`Images · ${getTitle(page)}`} onClose={onClose} /><div className="min-h-0 flex-1 overflow-y-auto px-5 py-7 sm:px-9"><div className="grid gap-x-8 gap-y-7 sm:grid-cols-2 lg:grid-cols-4">{images.map((image) => <article key={image.url} className="min-w-0"><div className="relative aspect-square overflow-hidden rounded-[16px] border-[3px] border-white bg-[#dedede] shadow-[0_13px_5px_rgba(0,0,0,.01),0_7px_4px_rgba(0,0,0,.02),0_3px_3px_rgba(0,0,0,.03),0_1px_2px_rgba(0,0,0,.04)]"><Image src={image.url} alt={image.alt} fill unoptimized className="object-cover" /></div><h3 className="mt-4 font-serif text-[20px]">{image.title}</h3><p className="mt-2 text-[11px] font-medium leading-4 text-black/45">Alt : {image.alt}</p><p className="mt-1 text-[11px] text-black/35">Ajoutée {getRelativeDate(page.editorial?.createdAt).toLowerCase()}</p></article>)}{!images.length ? <p className="col-span-full py-20 text-center text-[13px] text-black/40">Aucune image générée pour cet article.</p> : null}</div></div><footer className="shrink-0 bg-gradient-to-t from-white via-white to-white/20 p-5 sm:px-9"><button type="button" onClick={onBack} className="flex h-12 items-center gap-2 rounded-[9px] bg-[#f3f3f3] px-5 text-[14px] font-semibold"><ArrowLeft size={18} />Retour à l’article</button></footer></>;
}

function PhaseOverlay({ kind, page, onBack, onClose }: { kind: "research" | "outline" | "writing"; page: SitePage; onBack: () => void; onClose: () => void }) {
  const research = page.editorial?.research;
  const outline = page.editorial?.outline;
  const article = page.editorial?.article;
  const fields = getArticleFields(page);
  const title = kind === "research" ? "Dossier de recherche" : kind === "outline" ? "Structure de l’article" : "Rédaction finale";
  return <><OverlayTop title={`${title} · ${getTitle(page)}`} onClose={onClose} /><div className="min-h-0 flex-1 overflow-y-auto px-5 py-8 sm:px-9">
    {kind === "research" ? research ? <div className="grid gap-7 text-[14px] leading-7 text-black/65"><TextSection title="Résumé" text={research.summary} /><div className="grid gap-4 md:grid-cols-3"><TextSection title="Intention de recherche" text={research.searchIntent} /><TextSection title="Audience" text={research.audience} /><TextSection title="Angle éditorial" text={research.angle} /></div>{research.performanceAnalysis ? <><TextSection title={`Analyse des anciennes pages · ${research.performanceAnalysis.dataStatus === "connected" ? "données connectées" : research.performanceAnalysis.dataStatus === "partial" ? "données partielles" : "données absentes"}`} text={research.performanceAnalysis.summary} /><ListSection title="Constats issus des performances" items={research.performanceAnalysis.mainFindings} /><ListSection title="Pages qui ont bien fonctionné" items={research.performanceAnalysis.winningPages.map((item) => `${item.path}\n${item.reason}`)} /><ListSection title="Pages faibles ou sous-exploitées" items={research.performanceAnalysis.weakPages.map((item) => `${item.path}\n${item.reason}`)} /><ListSection title="Opportunités Search Console" items={research.performanceAnalysis.seoOpportunities.map((item) => `${item.path}\n${item.opportunity}`)} /><ListSection title="Formats et angles à réutiliser" items={research.performanceAnalysis.contentPatterns} /></> : null}<ListSection title="Faits et sources transmis à l’agent de structure" items={research.facts.map((fact) => `${fact.claim}\n${fact.sourceTitle} — ${fact.sourceUrl}`)} /><ListSection title="Questions à traiter" items={research.questions} /><ListSection title="Mots-clés" items={research.keywords} inline /><ListSection title="Précautions" items={research.safetyNotes} /></div> : <EmptyPhase text="Aucun dossier de recherche n’a été conservé pour cet article." /> : null}
    {kind === "outline" ? outline ? <div className="grid gap-7 text-[14px] leading-7 text-black/65"><TextSection title="Titre proposé" text={outline.title} /><TextSection title="Résumé" text={outline.excerpt} /><div className="grid gap-4 md:grid-cols-4"><TextSection title="Catégorie" text={outline.category} /><TextSection title="Slug" text={outline.slug} /><TextSection title="Temps de lecture" text={outline.readingTime} /><TextSection title="Alt de l’image" text={outline.heroImageAlt} /></div><section><h3 className="font-serif text-[20px] text-[#1c1c1c]">Plan transmis à l’agent de rédaction</h3><div className="mt-4 grid gap-3">{outline.sections.map((section, index) => <div key={`${section.title}-${index}`} className="rounded-[14px] border border-black/[0.06] bg-[#fafafa] p-5"><p className="text-[11px] font-semibold uppercase tracking-[.08em] text-black/35">{section.level} · Partie {index + 1}</p><h4 className="mt-2 font-serif text-[18px] text-[#1c1c1c]">{section.title}</h4><p className="mt-2">{section.purpose}</p><ul className="mt-3 list-disc pl-5">{section.points.map((point) => <li key={point}>{point}</li>)}</ul></div>)}</div></section></div> : <EmptyPhase text="Aucune structure n’a été conservée pour cet article." /> : null}
    {kind === "writing" ? <WritingContent page={page} article={article} fields={fields} /> : null}
  </div><footer className="shrink-0 border-t border-black/[0.07] p-5 sm:px-9"><button type="button" onClick={onBack} className="flex h-12 items-center gap-2 rounded-[9px] bg-[#f3f3f3] px-5 text-[14px] font-semibold"><ArrowLeft size={18} />Retour à l’article</button></footer></>;
}

function WritingContent({ page, article, fields }: { page: SitePage; article: GeneratedArticle | undefined; fields: ArticleDetailFields | null }) {
  const blocks = article?.blocks ?? fields?.blocks.filter((block) => block.kind === "heading" || block.kind === "paragraph") ?? [];
  if (!blocks.length) return <EmptyPhase text="Aucune rédaction finale n’a été conservée pour cet article." />;
  return <article className="mx-auto max-w-[820px]"><p className="text-[11px] font-semibold uppercase tracking-[.1em] text-black/35">Texte final transmis au CMS</p><h2 className="mt-3 font-serif text-[30px] leading-tight">{article?.title ?? getTitle(page)}</h2><p className="mt-4 text-[15px] leading-7 text-black/55">{article?.excerpt ?? fields?.subtitle}</p><div className="mt-8">{blocks.map((block, index) => block.kind === "heading" ? block.level === "h3" ? <h4 key={index} className="mb-3 mt-7 text-[17px] font-semibold">{block.text}</h4> : <h3 key={index} className="mb-3 mt-9 font-serif text-[24px]">{block.text}</h3> : block.kind === "paragraph" ? <p key={index} className="mb-5 text-[15px] leading-8 text-black/65">{block.text}</p> : null)}</div></article>;
}

function TextSection({ title, text }: { title: string; text: string }) {
  return <section className="rounded-[14px] border border-black/[0.06] bg-[#fafafa] p-5"><h3 className="font-serif text-[18px] text-[#1c1c1c]">{title}</h3><p className="mt-2 whitespace-pre-wrap">{text}</p></section>;
}

function ListSection({ title, items, inline = false }: { title: string; items: string[]; inline?: boolean }) {
  return <section><h3 className="font-serif text-[20px] text-[#1c1c1c]">{title}</h3><div className={`mt-3 ${inline ? "flex flex-wrap gap-2" : "grid gap-2"}`}>{items.map((item, index) => inline ? <span key={`${item}-${index}`} className="rounded-full bg-[#ededed] px-3 py-1 text-[12px] text-black/55">{item}</span> : <p key={`${item}-${index}`} className="whitespace-pre-wrap rounded-[12px] border border-black/[0.06] bg-[#fafafa] px-4 py-3">{item}</p>)}</div></section>;
}

function EmptyPhase({ text }: { text: string }) {
  return <div className="grid min-h-[280px] place-items-center text-center"><div><Sparkles size={24} className="mx-auto text-black/15" /><p className="mt-3 text-[13px] text-black/40">{text}</p></div></div>;
}
