...
export function AssetLibrary({ project, initialAssets }: { project: DashboardProject; initialAssets: DashboardAsset[] }) {
  ...
  async function saveBrand() {
    ...
    try {
      ...
      setSavedBrand(brand); setMessage("Identité enregistrée. Les miniatures des articles sont régénérées avec la nouvelle couleur."); router.refresh();
    ...
    <div className="pb-8 font-[var(--font-inter)] sm:pb-16">
      ...
      <section className="mt-7 border-y border-black/[0.11] py-7 sm:mt-9 sm:py-9">
        ...
        <div className="mt-7 grid gap-0 divide-y divide-black/[.11] border-y border-black/[.11] lg:grid-cols-2 lg:divide-x lg:divide-y-0">
          ...
          <div className="space-y-5 p-5 sm:p-6"><div className="flex items-center gap-2"><ImagePlus size={17}/><h3 className="font-serif text-[20px]">Logos</h3></div><div className="grid gap-3 sm:grid-cols-2"><input ref={darkLogoInputRef} type="file" accept="image/png,image/svg+xml,image/webp,image/jpeg" className="hidden" onChange={(event) => void uploadLogo(event.target.files, "dark")}/><input ref={lightLogoInputRef} type="file" accept="image/png,image/svg+xml,image/webp,image/jpeg" className="hidden" onChange={(event) => void uploadLogo(event.target.files, "light")}/><div className="rounded-[9px] bg-black/[.025] p-3 text-[11px] font-semibold text-black/55">Logo clair — utilisé sur fond sombre<button type="button" onClick={() => darkLogoInputRef.current?.click()} className="mt-3 h-10 w-full rounded-[8px] border border-black/10 bg-white px-3 text-[12px] font-medium text-black">Importer le logo clair</button></div><div className="rounded-[9px] bg-black/[.025] p-3 text-[11px] font-semibold text-black/55">Logo sombre — utilisé sur fond clair<button type="button" onClick={() => lightLogoInputRef.current?.click()} className="mt-3 h-10 w-full rounded-[8px] border border-black/10 bg-white px-3 text-[12px] font-medium text-black">Importer le logo sombre</button></div></div></div>
        ...
        <div className="mt-5 rounded-[10px] bg-black/[.025] p-4 text-[12px] text-black/55">
          <div className="flex items-center gap-2 font-semibold">
            <MapPin size={16} />
            Avis Google
          </div>
          <div className="mt-4 grid gap-3 lg:grid-cols-[minmax(0,1.6fr)_minmax(120px,.5fr)_minmax(120px,.5fr)_minmax(140px,.6fr)]">
            <label className="min-w-0 font-semibold text-black/55">
              Lien de l’établissement
              <input value={brand.googleReviewsUrl ?? ""} onChange={(event) => updateBrand({ googleReviewsUrl: event.target.value })} placeholder="Lien Google Maps de l’établissement" className="mt-2 h-10 w-full rounded-[8px] border border-black/10 bg-white px-3 text-[12px] font-medium text-black outline-none"/>
            </label>
            <label className="min-w-0 font-semibold text-black/55">
              Libellé
              <input value={brand.googleReviewLabel ?? ""} onChange={(event) => updateBrand({ googleReviewLabel: event.target.value })} placeholder="Excellent" className="mt-2 h-10 w-full rounded-[8px] border border-black/10 bg-white px-3 text-[12px] font-medium text-black outline-none"/>
            </label>
            <label className="min-w-0 font-semibold text-black/55">
              Note
              <input value={brand.googleReviewScore ?? ""} onChange={(event) => updateBrand({ googleReviewScore: event.target.value })} placeholder="4,8/5" className="mt-2 h-10 w-full rounded-[8px] border border-black/10 bg-white px-3 text-[12px] font-medium text-black outline-none"/>
            </label>
            <label className="min-w-0 font-semibold text-black/55">
              Nombre d’avis
              <input value={brand.googleReviewCount ?? ""} onChange={(event) => updateBrand({ googleReviewCount: event.target.value })} placeholder="500 avis" className="mt-2 h-10 w-full rounded-[8px] border border-black/10 bg-white px-3 text-[12px] font-medium text-black outline-none"/>
            </label>
          </div>
          <p className="mt-3 text-[11px] leading-5 text-black/45">
            Ces valeurs sont la source unique utilisée sur toutes les pages. Elles pourront ensuite être remplacées automatiquement par la synchronisation Google Business Profile.
          </p>
        </div>
      </section>
