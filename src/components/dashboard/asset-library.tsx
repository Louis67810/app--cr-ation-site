    ...
    <div className="pb-8 font-[var(--font-inter)] sm:pb-16">
      ...
      <section className="mt-7 border-y border-black/[0.11] py-7 sm:mt-9 sm:py-9">
        ...
        <div className="mt-7 grid gap-0 divide-y divide-black/[.11] border-y border-black/[.11] lg:grid-cols-2 lg:divide-x lg:divide-y-0">
          <div className="space-y-5 p-5 sm:p-6"><div className="flex items-center gap-2"><Palette size={17}/><h3 className="font-serif text-[20px]">Couleurs</h3></div><div className="flex flex-wrap gap-5"><label className="flex items-center gap-3 text-[12px] font-medium">Fond principal<input aria-label="Couleur principale" type="color" value={brand.primaryColor} onChange={(event) => setBrand((current) => ({ ...current, primaryColor: event.target.value.toUpperCase(), accentColor: synchronizedAccent(event.target.value, current.accentColor) }))} className="size-12 cursor-pointer rounded-full border-2 border-white bg-transparent p-0 shadow"/><span className="text-black/45">{brand.primaryColor}</span></label><label className="flex items-center gap-3 text-[12px] font-medium">Accent<input aria-label="Couleur accent" type="color" value={brand.accentColor} onChange={(event) => updateBrand({ accentColor: event.target.value.toUpperCase() })} className="size-12 cursor-pointer rounded-full border-2 border-white bg-transparent p-0 shadow"/><span className="text-black/45">{brand.accentColor}</span></label></div></div>
          ...
          <div className="space-y-5 p-5 sm:p-6"><div className="flex items-center gap-2"><ImagePlus size={17}/><h3 className="font-serif text-[20px]">Logos</h3></div><div className="grid gap-3 sm:grid-cols-2"><label className="text-[11px] font-semibold text-black/55">Sur fond sombre<select value={brand.logoOnDarkUrl ?? ""} onChange={(event) => updateBrand({ logoOnDarkUrl: event.target.value })} className="mt-2 h-10 w-full rounded-[8px] border border-black/10 bg-white px-3 text-[12px] font-medium text-black"><option value="">Logo actuel</option>{assets.map((asset) => <option key={`dark-${asset.id}`} value={asset.public_url}>{asset.title}</option>)}</select></label><label className="text-[11px] font-semibold text-black/55">Sur fond clair<select value={brand.logoOnLightUrl ?? ""} onChange={(event) => updateBrand({ logoOnLightUrl: event.target.value })} className="mt-2 h-10 w-full rounded-[8px] border border-black/10 bg-white px-3 text-[12px] font-medium text-black"><option value="">Logo actuel</option>{assets.map((asset) => <option key={`light-${asset.id}`} value={asset.public_url}>{asset.title}</option>)}</select></label></div></div>
        ...
        <div className="mt-5 flex items-center gap-3 text-[12px] text-black/48"><MapPin size={16} className="text-black/55"/><span>Connexion Google Reviews — emplacement prêt pour relier vos avis.</span></div>
      </section>
