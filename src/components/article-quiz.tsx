...
export function ArticleQuiz({ quiz }: { quiz: ReusableQuiz }) {
  ...
  return (
    <div className="rounded-[21px] bg-[#003441] px-6 py-14 text-white md:px-12 lg:px-[51px] lg:py-[82px]">
      ...
      <div className="mx-auto mt-12 max-w-[786px]">
        ...
        <div className="mt-2 h-2 overflow-hidden rounded-full bg-white/[0.08]">
          <div className="h-full rounded-full bg-[#00d494] transition-all duration-500" style={{ width: `${progress}%` }} />
        </div>
      </div>
      ...
      <div className="mx-auto mt-10 max-w-[945px] rounded-3xl bg-[#063f4d] p-5 md:p-10">
        {complete ? (
          <div className="grid gap-6 md:grid-cols-[minmax(0,1fr)_minmax(220px,.72fr)] md:items-center">
            <div>
              {result?.category ? <span className="inline-flex rounded-full bg-[#00d494]/15 px-3 py-1 text-[12px] font-semibold text-[#65efc4]">{result.category}</span> : null}
              ...
              {result?.recommendations?.length ? <ul className="mt-5 grid gap-2 text-[14px] text-white/75">{result.recommendations.map((recommendation) => <li key={recommendation} className="flex gap-2"><Check size={17} className="mt-0.5 shrink-0 text-[#00d494]" />{recommendation}</li>)}</ul> : null}
          ...
          <div className="grid gap-7">
            ...
            <div className={`grid gap-4 ${visualChoices ? "md:grid-cols-2" : ""}`}>
              {options.map((option) => <button key={option.id} type="button" onClick={() => setSelectedId(option.id)} className={`group overflow-hidden rounded-[20px] border text-left transition ${selectedId === option.id ? "border-[#00d494] bg-white/[0.1]" : "border-white/[0.08] bg-white/[0.03] hover:bg-white/[0.07]"}`}>
                {option.imageUrl ? <span className="relative block aspect-[16/10] overflow-hidden bg-white/[0.05]"><Image src={option.imageUrl} alt={option.imageAlt ?? option.label} fill unoptimized className="object-cover transition duration-500 group-hover:scale-[1.025]" /><span className={`absolute right-4 top-4 grid size-7 place-items-center rounded-full border backdrop-blur ${selectedId === option.id ? "border-[#00d494] bg-[#00d494] text-[#003441]" : "border-white/40 bg-black/20 text-transparent"}`}><Check size={16} /></span></span> : null}
